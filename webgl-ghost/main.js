...
function initTexturesForExample() {
    textureArray.push({}) ;
    loadFileTexture(textureArray[textureArray.length-1],"metalbox.png") ;
...
window.onload = function init() {
    ...
    gl.clearColor( 0.47, 0.47, 0.47, 1.0 );
...
// Draws the ghost body
function drawGhostBody() {
    gRotate(5, 1.0, 0.0, 0.0);
    gPush();
        gl.bindTexture(gl.TEXTURE_2D, textureArray[1].textureWebGL);
        gl.uniform1i(gl.getUniformLocation(program, "useTextures"), 1);
        
        gTranslate( 0.0, 2.0, 0.0 );
        gScale( 1.25, 1.5, 1.25 );
        drawCube(); // Main body

        gPush();
            gTranslate( 0.0, -1.0, 0.0 );
            gScale( 0.8, 0.20, 0.8 );
            drawCube(); // Bottom segment 1

            gPush();
                gTranslate( 0.0, -1.0, 0.0 );
                gScale( 0.8, 0.75, 0.8 );
                drawCube(); // Bottom segment 2
            gPop();
        ...
        gPush();
            gTranslate( -0.5, 0.25, 0.8 );
            gScale( 0.25, 0.3, 0.25 );
...
// Draws a finger at a given position with bending
function drawFinger(x, y, z) {
    gPush();
        gTranslate(x, y, z);
        gRotate(35 * Math.sin(0.5 * TIME), 1, 0, 0);
        gScale(0.25, 0.25, 0.5);
        drawCube(); // Finger Base

        gTranslate(0.0, -0.25, 1.0);  // Move to middle joint
        gRotate(30 * Math.sin(0.5 * TIME), 1, 0, 0); // Bend at middle joint

        gPush();
            drawCube(); // Finger Middle
        gPop();

        gTranslate(0.0, -0.25, 1.0); // Move to tip joint
        gRotate(25 * Math.sin(0.5 * TIME), 1, 0, 0); // Extra bending at the tip
...
// Draws a hand at a given position
function drawHand(x, y, z) {
    gPush();
        gl.bindTexture(gl.TEXTURE_2D, textureArray[1].textureWebGL);
        gl.uniform1i(gl.getUniformLocation(program, "useTextures"), 1);
        gTranslate(x, y, z);
        ...
        // Draw fingers
        drawFinger(-0.75, 0.75, 1.0); // Index Finger
        drawFinger(0.75, 0.75, 1.0);  // Other Finger
...
// Draws both hands
function drawGhostHands() {
    gPush();
        gRotate(-10, 1.0, 0.0, 0.0);
        drawHand(2.0, 1.75, 2.0, false); // Right Hand
        drawHand(-2.0, 1.75, 2.0, true); // Left Hand

    gPop();
}

function animateGhostBody() {
    var ghostX = 0.0;
    var ghostY = 1.0 * Math.sin( 0.5 * TIME );
    var ghostZ = 1.0 * Math.sin( 0.5 * TIME );
    gTranslate( ghostX, ghostY, ghostZ );
}

function animateGhostHands() {
    var handsX = 0.0;
    var handsY = 1.5 * Math.sin( 0.5 * TIME );
    var handsZ = 2.0 * Math.sin( 0.5 * TIME );
    gTranslate( handsX, handsY, handsZ );
}
...
function render(timestamp) {
    ...
    let offsetX = 20 * Math.cos(0.1 * TIME)
    let offsetZ = 20 * Math.sin(0.1 * TIME)
    eye = vec3( offsetX, 5, offsetZ ); // 360 spin view
